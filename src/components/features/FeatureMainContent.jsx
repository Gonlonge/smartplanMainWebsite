...
const FeatureMainContent = forwardRef(function FeatureMainContent({ docId, slug }, fwdRef) {
  const [title, setTitle] = useState("");
  const [shortDesc, setShortDesc] = useState("");
  const [images, setImages] = useState([]);
  const [user, setUser] = useState(null);
  ...
  useEffect(() => {
    ...
    try {
      if (docId) {
        ...
        const q = query(collection(db, "features"), where("slug", "==", slug), limit(1));
        unsubQuery = onSnapshot(
          q,
          (qs) => {
            if (!qs.empty) {
  ...
  const uploadAll = async () => {
    ...
    const effectiveSlug = (slug || slugify(title) || "untitled").slice(0, 60);

    try {
      const urls = [];
      for (const file of selectedFiles) {
        const ext = file.name.split(".").pop() || "jpg";
        const path = `features/${effectiveSlug}/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
  ...
  const closeViewer = () => setViewerOpen(false);
  const prev = () => setViewerIndex((i) => (i - 1 + images.length) % images.length);
  const next = () => setViewerIndex((i) => (i + 1) % images.length);
    ...
    <Box sx={{ mb: 6 }}>
      <Grid container spacing={6} alignItems="flex-start">
        ...
        <Grid item xs={12} md={mainImage ? 6 : 12}>
          <Box>
            {loading ? (
              <Typography variant="body1" color="text.secondary">Laster…</Typography>
            ) : !edit ? (
              <>
                <Typography variant="h3" fontWeight={700} gutterBottom sx={{ pr: 12 }}>
                  {title || "Uten tittel"}
                </Typography>
                {shortDesc && (
                ...
                {user && (
                  <Stack direction="row" justifyContent="flex-end" sx={{ mt: 3 }}>
                    <Button size="medium" startIcon={<EditIcon />} onClick={() => setEdit(true)}>
                      Rediger
                    </Button>
                  </Stack>
                )}
              ...
              <Stack spacing={2} sx={{ pt: 4 }}>
                <Card variant="outlined">
                  <CardContent>
                    <Stack spacing={2}>
                      <Stack spacing={1}>
                        <Typography variant="subtitle2" color="text.secondary">Tittel</Typography>
                        <Box
                          component="input"
                          value={title}
                      ...
                      <Stack spacing={1}>
                        <Typography variant="subtitle2" color="text.secondary">Kort beskrivelse</Typography>
                        <Box
                          component="textarea"
                          value={shortDesc}
                ...
                <Card variant="outlined">
                  <CardContent>
                    <Stack spacing={2}>
                      <Typography variant="subtitle2" color="text.secondary">Bilder</Typography>
                      ...
                      >
                        <Button component="label" variant="outlined" startIcon={<AddPhotoAlternateIcon />} disabled={uploading} sx={{ mb: 1 }}>
                          Velg lokale bilder
                          <input hidden type="file" accept="image/*" multiple onChange={onPickFiles} />
                        </Button>
                        <Typography variant="body2" color="text.secondary">
                          PNG / JPG / WEBP • Flere filer støttes
                        </Typography>
                      </Box>
                      ...
                      {selectedFiles.length > 0 && (
                        <Stack spacing={1}>
                          <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                            {selectedFiles.map((f) => (
                              <Chip key={f.name} label={f.name} variant="outlined" sx={{ mb: 1 }} />
                            ))}
                          </Stack>
                            ...
                            <Stack spacing={1}>
                              {selectedFiles.map((f) => (
                                <Box key={f.name}>
                                  ...
                                  <LinearProgress variant="determinate" value={uploadProgress[f.name] ?? 0} sx={{ mt: 0.5 }} />
                                </Box>
                              ))}
                            </Stack>
                          )}
                          ...
                          <Stack direction="row" spacing={1}>
                            <Button onClick={uploadAll} disabled={!selectedFiles.length || uploading} variant="contained" startIcon={<UploadIcon />}>
                      ...
                      {images.length > 0 && (
                        <>
                          <Divider />
                          <Typography variant="subtitle2">Bilder i dokumentet</Typography>
                ...
                <Stack direction="row" spacing={1} justifyContent="flex-end">
                  <Button variant="contained" startIcon={<SaveIcon />} onClick={handleSave} disabled={saving || uploading}>
                    {saving ? "Lagrer…" : "Lagre endringer"}
                  </Button>
                  <Button variant="text" startIcon={<CloseIcon />} disabled={saving || uploading} onClick={() => setEdit(false)}>
                    Avbryt
                  </Button>
                </Stack>
              </Stack>
      ...
      <Dialog open={viewerOpen} onClose={closeViewer} maxWidth="lg" fullWidth>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ px: 2, pt: 1 }}>
          <Button onClick={prev} startIcon={<NavigateBeforeIcon />} disabled={images.length <= 1}>Forrige</Button>
          ...
          {images.length > 0 && (
            <>
              ...
              <Stack direction="row" justifyContent="space-between" sx={{ mt: 1 }}>
                <Button onClick={prev} startIcon={<NavigateBeforeIcon />} disabled={images.length <= 1}>Forrige</Button>
                <Button onClick={next} endIcon={<NavigateNextIcon />} disabled={images.length <= 1}>Neste</Button>
              </Stack>
            </>
          )}
        </DialogContent>
